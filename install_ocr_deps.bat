...
echo.
echo 正在安装其他依赖...
python -m pip install pillow pypdfium2 pypdf pdfplumber numpy opencv-python

echo.
echo [4/4] 安装可选 OCR 技术...
echo.
echo 是否安装 PaddleOCR? (需要较长时间)
choice /C YN /M "请选择 [Y/N]: "
if errorlevel 1 (
    echo 跳过 PaddleOCR 安装
) else (
    echo 正在安装 PaddleOCR...
...
echo.
echo 是否安装 Tesseract OCR? (需要单独安装 tesseract 可执行文件)
choice /C YN /M "请选择 [Y/N]: "
if errorlevel 1 (
    echo 跳过 Tesseract 安装
) else (

@echo off
chcp 65001 >nul 2>&1
title IFlow Agent Launcher (with vLLM OCR)

echo ====================================================
echo    IFlow Agent Launcher (with vLLM OCR)
echo ====================================================
echo.
...
:: Start vLLM OCR Service
echo.
echo [1/5] Starting vLLM OCR Service...
echo [INFO] vLLM OCR will run on port 8000
echo [INFO] Note: Backend will use port 8001 instead
echo.
echo [WARNING] vLLM OCR requires Python 3.12+
echo [WARNING] First run will download the model (~2GB)
echo.
start "vLLM OCR" cmd /k "cd /d %BASE_DIR% && start_vllm_ocr.bat"

echo Waiting 15 seconds for vLLM OCR to start...
ping 127.0.0.1 -n 16 >nul

:: Start Backend (on port 8001 to avoid conflict with vLLM)
echo.
echo [2/5] Starting Backend (with hot reload) on port 8001...
start "Backend" cmd /k "cd /d %BASE_DIR% && set PYTHONPATH=%BASE_DIR% && python -m uvicorn backend.server:app --host 0.0.0.0 --port 8001 --reload"
...
echo.
echo [4/5] Starting Node.js Server...
...
echo    vLLM OCR (Python): http://localhost:8000
...
echo ====================================================
echo.
echo [INFO] vLLM OCR is now available for resume upload!

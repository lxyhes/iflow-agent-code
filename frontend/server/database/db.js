...
const runMigrations = () => {
  ...
  const migrate = db.transaction(() => {
    ...
    if (!columnNames.has('git_name')) {
      console.log('Running migration: Adding git_name column');
      db.prepare('ALTER TABLE users ADD COLUMN git_name TEXT').run();
    }

    if (!columnNames.has('git_email')) {
      console.log('Running migration: Adding git_email column');
      db.prepare('ALTER TABLE users ADD COLUMN git_email TEXT').run();
    }

    if (!columnNames.has('has_completed_onboarding')) {
      console.log('Running migration: Adding has_completed_onboarding column');
      db.prepare('ALTER TABLE users ADD COLUMN has_completed_onboarding BOOLEAN DEFAULT 0').run();
    }
  });
